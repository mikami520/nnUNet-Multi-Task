...
LastEditTime : 2025-01-23 00:57:39
FilePath     : /Documents/nnUNet/nnunetv2/inference/predict_from_raw_data.py
Description  :
I Love IU
...
class nnUNetPredictor(object):
    ...
    def pad_tensor(self, tensor, target_shape):
        """
        Pads a tensor to the target_shape.

        Args:
            tensor (torch.Tensor): The input tensor to pad.
            target_shape (Tuple[int, ...]): The desired shape.

        Returns:
            torch.Tensor: The padded tensor.
        """
        padding = []
        for dim, target in zip(reversed(tensor.shape[1:]), reversed(target_shape)):
            pad = max(target - dim, 0)
            padding.extend([0, pad])  # Pad on the end of each dimension
        return F.pad(tensor, padding, mode="constant", value=0)

    def batch_tensors(self, tensors):
        """
        Pads and stacks a list of tensors.

        Args:
            tensors (List[torch.Tensor]): List of tensors to batch.

        Returns:
            torch.Tensor: Batched tensor.
        """
        # Determine the maximum shape
        max_shape = torch.tensor([t.shape[1:] for t in tensors]).max(0)[0]
        # Pad tensors
        padded_tensors = [self.pad_tensor(t, max_shape.tolist()) for t in tensors]
        # Stack tensors
    ...
    def _internal_predict_sliding_window_return_logits(
        ...
        try:
            ...
            for i in tqdm(
                range(0, len(slicers), batch_size), disable=not self.allow_tqdm
            ):
                batch_slicers = slicers[i : i + batch_size]
                batch_data = torch.stack([data[sl] for sl in batch_slicers]).to(
                    self.device
                )
                batch_predictions = self._internal_maybe_mirror_and_predict(batch_data)
                ...
                if self.use_gaussian:
                    batch_predictions *= gaussian

                for sl, pred in zip(batch_slicers, batch_predictions):
    ...
    def predict_sliding_window_return_logits(
        ...
        with torch.no_grad():
            ...
            # allow mixture of fp16 and fp32 for the model inference
            with (
