...
LastEditTime : 2025-01-19 00:52:48
FilePath     : /Documents/nnUNet/nnunetv2/utilities/plans_handling/plans_handler.py
Description  :
I Love IU
...
class ConfigurationManager(object):
    def __init__(self, configuration_dict: dict):
        ...
        if "architecture" not in self.configuration.keys():
            ...
            if unet_class_name == "PlainConvUNet":
                ...
                raise RuntimeError(
                    f"Unknown architecture {unet_class_name}. This conversion only supports "
            ...
            arch_dict = {
                ...
                "arch_kwargs": {
                    ...
                    "features_per_stage": [
                        min(
                            self.configuration["UNet_base_num_features"] * 2**i,
                    ...
                    convs_or_blocks: deepcopy(
                        self.configuration["n_conv_per_stage_encoder"]
                    ),
                    "n_conv_per_stage_decoder": deepcopy(
                        self.configuration["n_conv_per_stage_decoder"]
                    ),
                    "conv_bias": True,
            ...
            del (
                ...
                self.configuration["UNet_base_num_features"],
                self.configuration["n_conv_per_stage_encoder"],
                self.configuration["n_conv_per_stage_decoder"],
                self.configuration["num_pool_per_axis"],
                self.configuration["pool_op_kernel_sizes"],
                self.configuration["conv_kernel_sizes"],
                self.configuration["unet_max_num_features"],
            )
    ...
    @property
    def network_arch_conv_name(self) -> str:
        return self.configuration["architecture"]["arch_kwargs"]["conv_op"].split(".")[
            -1
        ]
